<#
.SYNOPSIS
    Enhanced License Monitoring System with improved error handling, performance and credential security
.DESCRIPTION
    This script monitors software license expiration dates, sends alerts, and generates reports.
    It includes robust error handling, input validation, and performance optimizations.
.NOTES
    Version: 4.2
    Author: billel-eren
...
param (
    ...
    [string]$ToAddress = "",
    [string[]]$CCAddresses = @(),
    #  [string[]]$CCAddresses = @("additional1@example.com", "additional2@example.com"), 
    [string]$SMTPServer = "",
    [int]$SMTPPort = 587,
    
    # Secure Credential Handling
    [string]$CredentialPath = "$BasePath\email_credential.xml",
...
function Get-SecureCredential {
    ...
    try {
        if (Test-Path $CredentialPath) {
            ...
            # Prompt for credentials and save them securely if they don't exist
            $credential = Get-Credential -Message "Entrez vos identifiants SMTP" -UserName $FromAddress
            ...
            Write-Log "Identifiants enregistrés de manière sécurisée dans $CredentialPath" "INFO"
            return $credential
        }
    }
    catch {
        Write-Log "Erreur lors de la gestion des identifiants: $($_.Exception.Message)" "ERROR"
        throw
    }
}
...
function Remove-SecureCredential {
    ...
    try {
        if (Test-Path $CredentialPath) {
            ...
            Write-Log "Identifiants supprimés de $CredentialPath" "INFO"
        }
    }
    catch {
        Write-Log "Erreur lors de la suppression des identifiants: $($_.Exception.Message)" "ERROR"
    }
}
...
try {
    ...
    # Get or create secure credentials
    $EmailCredential = Get-SecureCredential -CredentialPath $CredentialPath
    ...
    # Clean up sensitive resources
    if ($EmailCredential) {
        $EmailCredential.Password.MakeReadOnly()
